...
$(document).bind('pagecreate',function(){
		...
		$(filesJSONObj.files).each(function(idx,file) {
			if (file.filename == absFilename) { 
				...
				if (idx != 0) {
					$('div#pagenav').append('<a  rel="external" href="'+filesJSONObj.files[idx-1].filename+'" data-icon="arrow-l" data-role="button" data-icon-pos="left">'+filesJSONObj.files[idx-1].short+'</a>');
				}
			
				$('div#pagenav').append('<a  rel="external" href="'+file.filename+'" data-role="button" data-icon="" class="ui-btn-active">'+file.title+'</a>');
		
				if (idx+1 != numFiles) {
					$('div#pagenav').append('<a rel="external" href="'+filesJSONObj.files[idx+1].filename+'" data-role="button" data-icon="arrow-r" data-iconpos="right">'+filesJSONObj.files[idx+1].short+'</a>');
				}
			}
		});
